$(document).ready(() => {
    ...
    $('.output').click((e) => {
        if (e.target.classList.contains('delete') && confirm('are you sure to remove this purchase: ')) {
        ...
        if (e.target.classList.contains('modifay') && confirm('are you sure to edit this purchase: ')) {
            ...
            //display data of purchase in form
                ...
                .then((data) => {
                    data.forEach((purchase) => {
                        if (purchase._id == idOfEtid) {
                            ...
                            $('input[name="friendCkeck"]').prop('checked', false);

                            purchase.ckeckFriends.forEach((friend) => {
                                if (friend.Consumption == 1) {
                                    $(`input[type="checkbox"][value="${friend.username}"]`).prop('checked', true);
                                }
                            });
                        }
                    });
            ...
            $('.add-row').text('edit');
            $('.add-update-purchase-form').attr('action', `/room/${id_room}/update_purchase`);
            $('.add-update-purchase-form').append(`<input type="hidden" name="id_purchase" value="${idOfEtid}">`);
        }
    })
